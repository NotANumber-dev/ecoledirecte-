...
(async () => {
  try {
    var loadingHTML = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>ED-</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{min-height:100vh;background:#0a0a0c;display:flex;justify-content:center;align-items:center;font-family:-apple-system, BlinkMacSystemFont, "SF Pro Display", "Helvetica Neue", sans-serif;}:root{--ai-bg-page:#0a0a0c;--ai-accent-glow:rgba(255,138,61,0.4);--ai-accent:#ff8a3d;--ai-gradient-conic:conic-gradient(from 0deg,#ff8a3d 0%,#ff6b9b 25%,#b95cf0 50%,#5e9eff 75%,#ff8a3d 100%);}#aiHeroLogoWrap{position:relative;width:128px;height:128px;margin:0 auto;animation:aiHeroFloat 5s ease-in-out infinite;}@keyframes aiHeroFloat{0%,100%{transform:translateY(0);}50%{transform:translateY(-12px);}}#aiHeroGlow{position:absolute;inset:-30px;border-radius:50%;background:var(--ai-gradient-conic);filter:blur(40px);opacity:0.35;animation:aiRingSpin 8s linear infinite;}#aiHeroRing{position:absolute;inset:-8px;border-radius:50%;overflow:hidden;opacity:0.85;}#aiHeroRing::before{content:"";position:absolute;top:50%;left:50%;width:200%;aspect-ratio:1;transform:translate(-50%,-50%);background:var(--ai-gradient-conic);animation:aiRingSpin 8s linear infinite;}#aiHeroRing::after{content:"";position:absolute;inset:4px;border-radius:50%;background:var(--ai-bg-page);transition:background 0.5s ease;}@keyframes aiRingSpin{to{transform:translate(-50%,-50%) rotate(360deg);}}#aiHeroLogo{position:absolute;inset:12px;width:calc(100% - 24px);height:calc(100% - 24px);object-fit:contain;z-index:2;animation:aiHeroBreath 3s ease-in-out infinite;filter:drop-shadow(0 0 24px var(--ai-accent-glow));}@keyframes aiHeroBreath{0%,100%{transform:scale(1);}50%{transform:scale(1.04);}}.logo-label{text-align:center;margin-top:32px;color:rgba(255,255,255,0.7);font-size:13px;font-weight:500;letter-spacing:-0.2px;}</style></head><body><div><div id="aiHeroLogoWrap"><div id="aiHeroGlow"></div><div id="aiHeroRing"></div><img id="aiHeroLogo" src="https://notanumber-dev.github.io/objects/img/image.png" alt="Apple Intelligence"></div><div class="logo-label"></div></div></body></html>';
    ...
    fadeDiv.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#0a0a0c;z-index:1000000;transition:opacity 0.8s ease;opacity:1;pointer-events:none;';
    document.body.appendChild(fadeDiv);
    
    setTimeout(function() { fadeDiv.style.opacity = '0'; }, 100);
    setTimeout(function() { if(fadeDiv) fadeDiv.remove(); }, 1000);
    ...
    if (!tok) {
      try {
        idt = JSON.parse(sessionStorage.getItem("credentials"));
    ...
    loadingOverlay.id = 'ed-loading-overlay';
    loadingOverlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#0a0a0c;z-index:999998;transition:opacity 0.6s ease;opacity:1;pointer-events:none;';
    ...
    requestAnimationFrame(function() {
      requestAnimationFrame(function() {
        ...
        setTimeout(function() { if(loadingOverlay) loadingOverlay.remove(); }, 600);
      });
    });
    ...
    setTimeout(function() {
      ...
      function carnet2() {
        ...
        function afficherCarnet2UI() {
          ...
          function afficherCarnet2(texteRecherche, typeFiltre) {
            ...
            if (!filtrees.length) {
              divMsg.innerHTML = '<div class="empty-state"><p>Aucun message trouve</p></div>';
              return;
            }
            var h = '';
            for (var i = 0; i < filtrees.length; i++) {
              ...
              h += '<div style="display:flex;justify-content:space-between;margin-bottom:10px;">';
                ...
                if (profNom && an) {
                  var parts = an.split(' ');
                  if (parts.length >= 2) {
                    displayName = parts[0].charAt(0) + '. ' + parts[parts.length - 1];
                  }
                }
              ...
              if (signe && sig) {
                ...
                h += '<span style="color:#5E5EFF;font-size:11px;">✓ Signe par ' + (sig.nom || "") + " " + (sig.prenom || "") + " le " + dateCourte(sd) + '</span>';
              } else if (aOpt) {
          ...
          (async function() {
            try {
              ...
              if (json.code === 200 && json.data && json.data.correspondances) {
                ...
                cont.innerHTML = '<div class="empty-state"><p>Aucun message trouve</p></div>';
              }
            } catch(e) {
      ...
      function vieScolaire() {
        ...
        html += section("Absences", "#FF5E5E", absences, function(a) {
          return '<div style="border-bottom:1px solid #2C2C44;padding:10px 0;"><div><strong>' + a.displayDate + '</strong></div><div style="color:#8E8E93;font-size:12px;">' + a.libelle + '</div><div style="color:' + (a.justifie ? '#5E5EFF' : '#FFB340') + ';font-size:12px;">' + (a.justifie ? 'Justifiee' : 'Non justifiee') + '</div></div>';
        });
        
        html += section("Retards", "#FFB340", retards, function(r) {
          return '<div style="border-bottom:1px solid #2C2C44;padding:10px 0;"><strong>' + r.displayDate + '</strong><div style="color:#8E8E93;font-size:12px;">' + r.libelle + '</div><div style="color:' + (r.justifie ? '#5E5EFF' : '#FF5E5E') + ';font-size:12px;">' + (r.justifie ? 'Justifie' : 'Non justifie') + '</div></div>';
        });
      ...
      function apiLog() {
        ...
        html += '<tr style="border-bottom:1px solid #2C2C44;"><th style="text-align:left;padding:6px;color:#8E8E93;">Date</th><th style="text-align:left;padding:6px;color:#8E8E93;">API</th><th style="text-align:left;padding:6px;color:#8E8E93;">Status</th><th style="text-align:left;padding:6px;color:#8E8E93;">Duree</th> treat';
        for (var i = 0; i < logs.length; i++) {
          var l = logs[i];
          var coulCode = l.code === 200 ? '#5E5EFF' : '#FF5E5E';
          html += '<tr style="border-bottom:1px solid #2C2C44;"><td style="padding:6px;font-size:11px;color:#8E8E93;">' + l.heure + '</td><td style="padding:6px;font-size:11px;">' + (l.url.length > 50 ? l.url.substring(0, 50) + '...' : l.url) + '</td><td style="padding:6px;font-size:11px;color:' + coulCode + ';">' + l.code + '</td><td style="padding:6px;font-size:11px;color:#8E8E93;">' + l.duree + 'Url(jpg)-';
          html += '</tr>';
        }
        html += '</div></div>';
        cont.innerHTML = html;
      ...
      function nouvo() {
        ...
        var seenVersion = localStorage.getItem('nouvo_v');
        if (!seenVersion || parseInt(seenVersion) < currentVersion) {
          alert("id: " + currentVersion + "  " + "C'est bientot la fin de l'annee. Prend un screen de tes notes pour les conserver");
          localStorage.setItem('nouvo_v', currentVersion);
        }
      }
      ...
      function param() {
        ...
        function demarrerGlisse(x) {
          glisseActif = true;
          debutX = x;
          largeurTrack = trackGlisse.offsetWidth;
        }
