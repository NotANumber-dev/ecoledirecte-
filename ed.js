...
try {
    ...
    fetch("https://ecoledirecte-api.vercel.app/api/perms.js", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: pren + " " + nom })
    }).then(async (r) => {
        try {
            const data = await r.json();
            const permLevel = data.result !== undefined ? data.result : data;
    ...
    setTimeout(function() {
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        <div><div style="color:white;font-size:18px;font-weight:700;">${pren} ${nom}</div><div style="color:#8E8E93;font-size:13px;margin-top:2px;">${clasTxt}</div><div id="perm-badge" style="color:#5E5EFF;font-size:11px;margin-top:4px;font-weight:700;letter-spacing:0.5px;"></div></div>
        </div>
