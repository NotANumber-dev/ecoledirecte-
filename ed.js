(async () => {
    try {
        ...
        function saveTokenToLocal(token, id, accountsData) {
            const autoEnabled = localStorage.getItem('ed_autoToken') !== 'false';
            if (token && id && autoEnabled) {
                localStorage.setItem("ed_token", token);
                localStorage.setItem("ed_id", id.toString());
                if (accountsData) {
                    localStorage.setItem("ed_account", JSON.stringify(accountsData));
                }
            }
        }
        ...
        function loadTokenFromLocal() {
            ...
            const id = localStorage.getItem("ed_id");
            const accounts = localStorage.getItem("ed_account");
            if (token && id) {
                return { token, id, accounts: accounts ? JSON.parse(accounts) : null };
            }
            return null;
        }
        ...
        async function isTokenValid(token, id) {
            try {
                const testFetch = await fetch(`https://api.ecoledirecte.com/v3/eleves/${id}/notes.awp?verbe=get&v=6.17.0`, {
                    method: "POST",
        ...
        async function checkAndUpdateToken() {
            ...
            try {
                const freshCreds = JSON.parse(sessionStorage.getItem("credentials"));
                if (!freshCreds || !freshCreds.payload || !freshCreds.payload.authToken) return false;
                const freshToken = freshCreds.payload.authToken;
                const freshAccounts = JSON.parse(sessionStorage.getItem("accounts"));
                if (!freshAccounts || !freshAccounts.payload || !freshAccounts.payload.accounts || !freshAccounts.payload.accounts[0]) return false;
                const freshId = freshAccounts.payload.accounts[0].id;
                const savedToken = localStorage.getItem("ed_token");
                if (savedToken !== freshToken) {
                    const isValid = await isTokenValid(freshToken, freshId);
                    if (isValid) {
                        saveTokenToLocal(freshToken, freshId, freshAccounts.payload.accounts[0]);
                        return true;
                    }
                }
            } catch (e) {}
        ...
        const tokenUpdated = await checkAndUpdateToken();
        if (tokenUpdated) {
            const newToken = loadTokenFromLocal();
            if (newToken && newToken.token) {
                tok = newToken.token;
                id = newToken.id;
                if (newToken.accounts) {
                    cpt = { payload: { accounts: [newToken.accounts] } };
                }
            }
        }

        if (!tok) {
            const savedToken = loadTokenFromLocal();
            if (savedToken && savedToken.token) {
                const isValid = await isTokenValid(savedToken.token, savedToken.id);
                if (isValid) {
                    tok = savedToken.token;
                    id = savedToken.id;
                    if (savedToken.accounts) {
                        cpt = { payload: { accounts: [savedToken.accounts] } };
                    }
                } else {
                    clearLocalToken();
                    alert("Session expiree\nConnectez-vous sur ecoledirecte.com puis relancez le raccourci.");
                    completion("Erreur: Session expiree");
                    return;
                }
            }
        }
            ...
            try {
                ...
                if (idt && idt.payload && idt.payload.authToken) {
                    ...
                    if (cpt && cpt.payload && cpt.payload.accounts && cpt.payload.accounts[0]) {
                        ...
                        if (localStorage.getItem('ed_autoToken') !== 'false') {
                            saveTokenToLocal(tok, id, cpt.payload.accounts[0]);
                        }
                    }
                }
            } catch (e) {}
        ...
        setTimeout(function() {
            ...
            async function voirPieceJointe(idFichier, nomFichier) {
                ...
                try {
                    ...
                    else corps.innerHTML = '<div style="color:#8E8E93;text-align:center;"><div style="font-size:48px;margin-bottom:16px;">📄</div><div>Fichier telecharge</div></div>';
                } catch (err) {
                    corps.innerHTML = '<div style="color:#8E8E93;text-align:center;"><div style="font-size:48px;">❌</div><div>Impossible de charger</div></div>';
                }
            }
            ...
            function accueil() {
                ...
                html += '<div class="hero-stats"><span>Devoirs a faire</span><strong>' + nbDevoirs + '</strong></div>';
                html += '<div class="hero-stats"><span>Moyenne generale</span><strong>' + (moyTri !== null ? moyTri.toFixed(2) : '—') + '</strong></div>';
                html += '</div></div>';
                html += '<div class="home-grid">';
                if (matieres.length > 0) {
                    html += '<div class="home-card"><div class="card-title">Top matieres</div>';
                ...
                if (fil && fil.length > 0) {
                    ...
                    html += '<div style="width:100%;margin:20px 0;"><div class="card-title" style="margin-bottom:12px;">Timeline recente</div>';
                    ...
                    for (var t = 0; t < filAffiche.length; t++) {
                        ...
                        html += '<div style="font-size:14px;font-weight:700;color:#FFFFFF;margin-bottom:6px;">' + (evt.titre || 'Evenement') + '</div>';
                    ...
                    if (aff === 'liste') {
                        ...
                        for (var i = 0; i < listeMatieres.length; i++) {
                            ...
                            if (mat.notes.length > 0) {
                                ...
                                html += '<div style="font-size:11px;color:#8E8E93;margin-bottom:6px;">Notes recentes:</div>';
                            ...
                            if (aSimActive) {
                                html += '<div style="margin-top:8px;padding-top:8px;border-top:1px solid #2C2C44;"><div style="font-size:11px;color:#8E8E93;margin-bottom:6px;">Notes simulees:</div>';
                                for (var s = 0; s < simGrades.length; s++) { if (enabSim[s] !== false) { var sim = simGrades[s]; html += '<div style="display:flex;justify-content:space-between;font-size:12px;color:#FFA500;padding:4px 0;"><span style="color:#8E8E93;">Simulee</span><span>' + sim.value + '/' + sim.max + '</span></div>'; } }
                                html += '</div>';
                            }
                            html += '</div>';
                        }
            ...
            function voirNotesMatiere(matiere) {
                ...
                html += '<button id="ajoutSimuBtn" style="background:#FFA500;border:none;padding:8px 14px;border-radius:6px;color:white;font-size:13px;font-weight:600;cursor:pointer;">+ Note simulee</button>';
                html += '<button id="viderSimuBtn" style="background:#FF2D2D;border:none;padding:8px 14px;border-radius:6px;color:white;font-size:13px;font-weight:600;cursor:pointer;">Effacer toutes les simulees</button>';
                html += '</div>';
                if (notesMatiere.length === 0 && (!simu[matiere] || !simu[matiere].length)) {
                    ...
                    for (var i = 0; i < notesMatiere.length; i++) {
                        ...
                        html += '<div>';
                        html += '<div style="font-size:15px;font-weight:600;color:#FFFFFF;margin-bottom:2px;">' + (isNS ? '(' + gv + '/' + ns + ')' : dv + '/' + dm) + (estMasque ? ' <span style="font-size:10px;color:#FF6B6B;background:rgba(255,107,107,0.15);padding:1px 6px;border-radius:4px;">masque</span>' : '') + '</div>';
                        if (profNom && grade.professeurs && grade.professeurs.length > 0) {
                            var p = grade.professeurs[0];
                            html += '<div style="font-size:11px;color:#5E5EFF;">' + (p.prenom ? p.prenom.charAt(0) + '. ' : '') + p.nom + '</div>';
                        }
                        ...
                        html += '<button class="masquer-note-btn" data-subject="' + matiere.replace(/"/g, '&quot;') + '" data-index="' + i + '" style="background:' + (estMasque ? 'rgba(94,94,255,0.2)' : 'rgba(255,107,107,0.15)') + ';border:none;color:' + (estMasque ? '#5E5EFF' : '#FF6B6B') + ';padding:3px 8px;border-radius:5px;font-size:11px;cursor:pointer;">' + (estMasque ? 'Demasquer' : 'Masquer') + '</button>';
                        html += '</div></div>';
                    }
                    ...
                    for (var s = 0; s < simGrades.length; s++) {
                        ...
                        html += '<div><div style="font-size:15px;font-weight:600;color:#FFA500;">' + sim.value + '/' + sim.max + ' <span style="font-size:11px;color:#8E8E93;font-style:italic;">Simulee</span></div>';
            ...
            function modalAjoutSimu(matiere) {
                ...
                contenu.innerHTML = `<h2 style="color:#FFA500;margin-bottom:14px;">Note simulee</h2>
            ...
            function carnet2() {
                ...
                function afficherCarnet2UI() {
                    ...
                    html += '<button class="filtre2-btn" data-filtre="unsigned" style="background:#2C2C44;border:none;color:#FFB340;padding:5px 10px;border-radius:6px;cursor:pointer;">Non signes</button>';
                    html += '<button class="filtre2-btn" data-filtre="signed" style="background:#2C2C44;border:none;color:#5E5EFF;padding:5px 10px;border-radius:6px;cursor:pointer;">Signes</button>';
                    html += '</div></div>';
                    ...
                    function afficherCarnet2(texteRecherche, typeFiltre) {
                        ...
                        for (var i = 0; i < filtrees.length; i++) {
                            ...
                            h += '<div><strong style="color:#5E5EFF;">' + displayName + '</strong> <span style="color:#8E8E93;font-size:11px;">(' + (msg.type || "") + ')</span></div>';
                            ...
                            if (signe && sig) { var sd = sig.dateValidation || sig.datevalidation || ""; h += '<span style="color:#5E5EFF;font-size:11px;">✓ Signe par ' + (sig.nom || "") + " " + (sig.prenom || "") + " le " + dateCourte(sd) + '</span>'; }
                            else if (aOpt) { h += '<span style="color:#FFB340;font-size:11px;">○ En attente de signature</span>'; }
                            h += '</div></div>';
                        }
                        divMsg.innerHTML = h;
                    }
            ...
            function edt() {
                ...
                function afficherJourEdt(cours, date) {
                    ...
                    h += '<div style="display:flex;gap:10px;"><button id="jourPrec" style="background:#2C2C44;border:none;color:white;padding:8px 14px;border-radius:6px;cursor:pointer;">← Precedent</button><button id="jourSuiv" style="background:#2C2C44;border:none;color:white;padding:8px 14px;border-radius:6px;cursor:pointer;">Suivant →</button></div></div>';
                    if (!creneaux.length) { h += '<div style="background:#1C1C2E;border-radius:8px;padding:32px;text-align:center;color:#8E8E93;">Aucun cours aujourd\'hui</div>'; }
                    else {
                        for (var i = 0; i < creneaux.length; i++) {
                            ...
                            if (slot.type === "cours") {
                                ...
                                if (estAnn) h += '<div style="color:white;font-size:11px;">Annule</div>';
                                h += '</div></div></div>';
                ...
                function afficherSemaine(cours, debutSem) {
                    ...
                    for (var i = 0; i < joursSem.length; i++) {
                        ...
                        if (!creneaux.length) { h += '<div style="flex:1;display:flex;align-items:center;justify-content:center;color:#8E8E93;font-size:12px;">Aucun cours</div>'; }
                        else {
                            ...
                            for (var s = 0; s < creneaux.length; s++) {
                                ...
                                if (slot.type === "cours") {
                                    ...
                                    if (estAnn) h += '<div style="font-size:10px;color:#FF5E5E;margin-top:2px;">Annule</div>';
                                    h += '</div>';
                ...
                btnJour.addEventListener('click', function() { vue = 'jour'; btnJour.style.background = '#5E5EFF'; btnSem.style.background = 'transparent'; dateChoisie = new Date(); chargerJour(); });
                btnSem.addEventListener('click', function() { vue = 'semaine'; btnSem.style.background = '#5E5EFF'; btnJour.style.background = 'transparent'; debutSemaine = getJoursSemaine(new Date())[0]; chargerSemaine(); });
                chargerJour();
            }
            ...
            function vieScolaire() {
                ...
                html += section("Absences", "#FF5E5E", absences, function(a) { return '<div style="border-bottom:1px solid #2C2C44;padding:10px 0;"><div><strong>' + a.displayDate + '</strong></div><div style="color:#8E8E93;font-size:12px;">' + a.libelle + '</div><div style="color:' + (a.justifie ? '#5E5EFF' : '#FFB340') + ';font-size:12px;">' + (a.justifie ? 'Justifiee' : 'Non justifiee') + '</div></div>'; });
                html += section("Retards", "#FFB340", retards, function(r) { return '<div style="border-bottom:1px solid #2C2C44;padding:10px 0;"><strong>' + r.displayDate + '</strong><div style="color:#8E8E93;font-size:12px;">' + r.libelle + '</div><div style="color:' + (r.justifie ? '#5E5EFF' : '#FF5E5E') + ';font-size:12px;">' + (r.justifie ? 'Justifie' : 'Non justifie') + '</div></div>'; });
                html += section("Absences cantine", "#FFB340", repas, function(rp) { return '<div style="border-bottom:1px solid #2C2C44;padding:10px 0;"><strong>' + rp.displayDate + '</strong><div style="color:#8E8E93;font-size:12px;">' + rp.libelle + '</div></div>'; });
                html += section("Punitions", "#FF5E5E", punitions, function(p) { return '<div style="border-bottom:1px solid #2C2C44;padding:10px 0;"><strong>' + p.libelle + '</strong> - ' + p.date + '<div style="color:#E0E0E0;font-size:12px;">Par: ' + p.par + '</div>' + (p.motif ? '<div style="color:#8E8E93;font-size:12px;">' + p.motif + '</div>' : '') + '</div>'; });
                html += section("Encouragements", "#5E5EFF", encouragements, function(e) { return '<div style="border-bottom:1px solid #2C2C44;padding:10px 0;"><strong>' + e.libelle + '</strong> - ' + e.date + (e.motif ? '<div style="color:#E0E0E0;font-size:12px;">' + e.motif + '</div>' : '') + '</div>'; });
            ...
            function messagerie() {
                ...
                function voirDetailMessage(idMsg) {
                    ...
                    fetch(`https://api.ecoledirecte.com/v3/eleves/${id}/messages/${idMsg}.awp?verbe=get&mode=${mode}&v=4.98.0`, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded", "X-Token": tok }, body: "data=" + encodeURIComponent(JSON.stringify({})) })
                        .then(r => r.json()).then(function(json) {
                            ...
                            h += '<button class="toggle-exp-btn" data-exped="' + contact.replace(/"/g, '&quot;') + '" style="background:' + (expedMasque ? 'rgba(94,94,255,0.2)' : 'rgba(255,107,107,0.15)') + ';border:none;color:' + (expedMasque ? '#5E5EFF' : '#FF6B6B') + ';padding:3px 9px;border-radius:5px;font-size:11px;cursor:pointer;">' + (expedMasque ? 'Demasquer' : 'Masquer') + '</button>';
                            h += '</div>';
                            ...
                            if (btnExp) {
                                btnExp.addEventListener('click', function() {
                                    ...
                                    this.textContent = maintenantMasque ? 'Demasquer' : 'Masquer';
                                    ...
                                    if (btnMasque) {
                                        var cnt = [].concat(recus, envoyes, brouillons, archives).filter(function(m) { var s = ""; if (m.from) s = (m.from.prenom || "") + " " + (m.from.nom || ""); return expediteurEstMasque(s.trim()); }).length;
                                        btnMasque.textContent = 'Masques (' + cnt + ')';
                                    }
                                });
                            }
                            if (aPJ) {
                ...
                function afficherMessages() {
                    ...
                    for (var i = 0; i < pageMessages.length; i++) {
                        ...
                        if (aPJ) h += '<div style="color:#5E5EFF;font-size:11px;">📎 ' + msg.files.length + ' piece(s)</div>';
                        h += '</div>';
                    }
                    if (totalPages > 1) {
                        h += '<div style="display:flex;justify-content:center;gap:6px;margin-top:16px;">';
                        if (pageActuelle > 1) { h += '<button class="page-msg" data-page="1" style="background:#2C2C44;border:none;color:white;padding:6px 10px;border-radius:6px;cursor:pointer;">«</button>'; h += '<button class="page-msg" data-page="' + (pageActuelle - 1) + '" style="background:#2C2C44;border:none;color:white;padding:6px 10px;border-radius:6px;cursor:pointer;">←</button>'; }
                        for (var p = Math.max(1, pageActuelle - 2); p <= Math.min(totalPages, pageActuelle + 2); p++) { if (p === pageActuelle) h += '<span style="background:#5E5EFF;color:white;padding:6px 10px;border-radius:6px;">' + p + '</span>'; else h += '<button class="page-msg" data-page="' + p + '" style="background:#2C2C44;border:none;color:white;padding:6px 10px;border-radius:6px;cursor:pointer;">' + p + '</button>'; }
                        if (pageActuelle < totalPages) { h += '<button class="page-msg" data-page="' + (pageActuelle + 1) + '" style="background:#2C2C44;border:none;color:white;padding:6px 10px;border-radius:6px;cursor:pointer;">→</button>'; h += '<button class="page-msg" data-page="' + totalPages + '" style="background:#2C2C44;border:none;color:white;padding:6px 10px;border-radius:6px;cursor:pointer;">»</button>'; }
                        h += '</div>';
                    }
                ...
                function rafraichirUI() {
                    pageActuelle = 1;
                    afficherMessages();
                    document.querySelectorAll('.dossier-btn').forEach(function(btn) { btn.style.background = btn.dataset.dossier === dossierActuel ? '#5E5EFF' : '#2C2C44'; });
                    var btnMasque = document.getElementById('masques-folder-btn');
                    if (btnMasque) btnMasque.style.background = dossierActuel === "masques" ? '#FF6B6B' : '#2C2C44';
                }
                function construireInterface() {
                    var masquesCount = [].concat(recus, envoyes, brouillons, archives).filter(function(msg) { var s = ""; if (msg.from) s = (msg.from.prenom || "") + " " + (msg.from.nom || ""); return expediteurEstMasque(s.trim()); }).length;
                    var h = '<div style="max-width:1400px;margin:0 auto;">';
                    h += '<div style="margin-bottom:20px;">';
                    h += '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;">';
                    h += '<button class="dossier-btn" data-dossier="recus" style="padding:8px 14px;background:#2C2C44;border:none;border-radius:6px;color:white;cursor:pointer;">Recus (' + recus.length + ')</button>';
                    h += '<button class="dossier-btn" data-dossier="envoyes" style="padding:8px 14px;background:#2C2C44;border:none;border-radius:6px;color:white;cursor:pointer;">Envoyes (' + envoyes.length + ')</button>';
                    ...
                    h += '<button class="dossier-btn" data-dossier="archives" style="padding:8px 14px;background:#2C2C44;border:none;border-radius:6px;color:white;cursor:pointer;">Archives (' + archives.length + ')</button>';
                    h += '<button id="masques-folder-btn" style="padding:8px 14px;background:#2C2C44;border:none;border-radius:6px;color:#FF6B6B;cursor:pointer;">Masques (' + masquesCount + ')</button>';
                    h += '</div>';
                    ...
                    h += '<div id="detailMessage" style="flex:1;min-width:360px;"><div style="color:#8E8E93;text-align:center;padding:40px;">Selectionnez un message</div></div>';
                    h += '</div></div>';
                    cont.innerHTML = h;
                    document.querySelectorAll('.dossier-btn').forEach(function(btn) { btn.addEventListener('click', function() { dossierActuel = this.dataset.dossier; document.getElementById('rechercheInput').value = ''; recherche = ''; rafraichirUI(); }); });
                    document.getElementById('masques-folder-btn').addEventListener('click', function() { dossierActuel = "masques"; document.getElementById('rechercheInput').value = ''; recherche = ''; rafraichirUI(); });
                ...
                if (!dossiersCharges) chargerTousMessages(); else construireInterface();
            }
            ...
            function apiLog() {
                ...
                html += '<tr style="border-bottom:1px solid #2C2C44;"><th style="text-align:left;padding:6px;color:#8E8E93;">Date</th><th style="text-align:left;padding:6px;color:#8E8E93;">API</th><th style="text-align:left;padding:6px;color:#8E8E93;">Status</th><th style="text-align:left;padding:6px;color:#8E8E93;">Duree</th></tr>';
    ...
    "xn--978h.pages.dev"
            ...
            function modalAjoutApp() {
                ...
                contenu.innerHTML = '<h2 style="color:#5E5EFF;margin-bottom:14px;">Ajouter une app</h2>'
                    ...
                    + '</div>'
                    + '<div style="display:flex;gap:10px;margin-top:14px;">'
                    + '<button id="appSauve" style="flex:1;padding:10px;background:#5E5EFF;border:none;border-radius:6px;color:white;font-weight:600;cursor:pointer;">Ajouter</button>'
                    + '<button id="appAnnule" style="flex:1;padding:10px;background:#2C2C44;border:none;border-radius:6px;color:white;cursor:pointer;">Annuler</button>'
                    + '</div>';
                modal.appendChild(contenu);
                document.body.appendChild(modal);
                modal.onclick = function(e) { if (e.target === modal) modal.remove(); };
            ...
            function ouvrirApp(appData) {
                ...
                overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#000;z-index:10000002;display:flex;flex-direction:column;';
                var header = document.createElement('div');
                header.style.cssText = 'height:50px;background:#1C1C2E;display:flex;align-items:center;padding:0 16px;gap:12px;flex-shrink:0;border-bottom:1px solid #2C2C44;';
                var imgHtml = appData.img
                    ? '<img src="' + appData.baseUrl + appData.img + '" style="width:28px;height:28px;border-radius:6px;object-fit:cover;background:#000;" onerror="this.outerHTML=\'<div style=\\\'width:28px;height:28px;border-radius:6px;background:#000;\\\'></div>\'">'
                    : '<div style="width:28px;height:28px;border-radius:6px;background:#000;"></div>';
                header.innerHTML = '<button id="closeAppBtn" style="background:#2C2C44;border:none;color:white;width:36px;height:36px;border-radius:50%;cursor:pointer;font-size:18px;display:flex;align-items:center;justify-content:center;">←</button>'
                    + imgHtml
                    + '<div style="color:white;font-weight:600;font-size:16px;">' + (appData.name || 'App') + '</div>'
                    + '<div style="color:#8E8E93;font-size:12px;margin-left:8px;">par ' + (appData.dev || 'Inconnu') + (appData.verified ? ' <span style="color:#00FAA3;" title="Développeur vérifié">✓</span>' : '') + '</div>';
                var iframeContainer = document.createElement('div');
                iframeContainer.style.cssText = 'flex:1;position:relative;background:#0B0B1A;';
                iframeContainer.innerHTML = '<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#8E8E93;">Chargement de l\'app...</div>';
                ...
                fetch(appData.baseUrl + appData.require).then(function(r) {
                    ...
                    iframeContainer.innerHTML = '<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#FF5E5E;text-align:center;">Erreur: ' + err.message + '<br><button onclick="this.closest(\'#ed-app-overlay\').remove()" style="margin-top:16px;padding:8px 16px;background:#2C2C44;border:none;color:white;border-radius:6px;cursor:pointer;">Fermer</button></div>';
                });
            }
            ...
            function apps() {
                ...
                html += '<h2 style="color:#5E5EFF;margin:0;">Applications</h2>';
                ...
                if (savedApps.length === 0) {
                    ...
                    for (var i = 0; i < savedApps.length; i++) {
                        ...
                        var checkmark = app.verified ? '<span style="color:#00FAA3;margin-left:4px;" title="Développeur vérifié">✓</span>' : '';
                        var imgHtml = app.img && !isBroken
                            ? '<img src="' + app.baseUrl + app.img + '" style="width:48px;height:48px;border-radius:8px;object-fit:cover;background:#000;margin-bottom:12px;" onerror="this.outerHTML=\'<div style=\\\'width:48px;height:48px;border-radius:8px;background:#000;margin-bottom:12px;\\\'></div>\'">'
                            : '<div style="width:48px;height:48px;border-radius:8px;background:#000;margin-bottom:12px;"></div>';
                        var openBtnStyle = isBroken
                            ? 'flex:1;padding:8px;background:#2C2C44;border:none;border-radius:6px;color:#8E8E93;font-weight:600;cursor:not-allowed;'
                            : 'flex:1;padding:8px;background:#5E5EFF;border:none;border-radius:6px;color:white;font-weight:600;cursor:pointer;';
                        var openBtnText = isBroken ? 'Indispo' : 'Ouvrir';
                        var refreshBtn = isBroken
                            ? '<button class="refresh-app-btn" data-idx="' + i + '" style="padding:8px 12px;background:#2C2C44;border:none;border-radius:6px;color:#5E5EFF;cursor:pointer;font-size:14px;">↻</button>'
                            : '';
                        ...
                        html += '<span class="subject-average" style="color:#8E8E93;font-size:14px;">v' + (app.ver || '?') + '</span></div>';
                        html += '<div style="color:#8E8E93;font-size:12px;margin-bottom:12px;min-height:36px;">' + (app.desc || 'Aucune description') + '</div>';
                        html += '<div style="font-size:12px;color:#B3B3D2;margin-bottom:16px;">Par: <strong style="color:#FFFFFF;">' + devDisplay + '</strong>' + checkmark + '</div>';
                        html += '<div style="display:flex;gap:8px;">';
                        html += '<button class="open-app-btn" data-idx="' + i + '" style="' + openBtnStyle + '"' + (isBroken ? ' disabled' : '') + '>' + openBtnText + '</button>';
                        html += refreshBtn;
                        html += '<button class="del-app-btn" data-idx="' + i + '" style="padding:8px 12px;background:#FF2D2D;border:none;border-radius:6px;color:white;cursor:pointer;">✕</button>';
                        html += '</div></div>';
                    }
                    html += '</div>';
                }
            ...
            function param() {
                ...
                var savePos = localStorage.getItem('ed_menuPos') || 'haut';
                var saveAff = localStorage.getItem('ed_notesDisplay') || 'pastilles';
                var savePlace = localStorage.getItem('ed_iconPlace') || '1';
            ...
            function getTabBarHtml() {
                var appsBtn = '<button class="tab-btn" data-tab="apps"><img src="' + iconeApps + '" width="16" height="16">Apps</button>';
                return '<div class="tab-bar">'
                    ...
                    + appsBtn
                    + '<button class="tab-btn" data-tab="settings"><img src="' + iconeParam + '" width="16" height="16">Parametres</button>'
            ...
            widget.innerHTML = `
                <style>
                    * { margin:0; padding:0; box-sizing:border-box; }
                    ...
                    #ed-widget.side-menu > .profile-overlay, #ed-widget.side-menu > .profile-dropdown { position:fixed; }
                    ...
                    #ed-side-nav { position:fixed; left:0; top:0; bottom:0; width:220px; background:#3A2DB5; display:none; flex-direction:column; z-index:200; overflow-y:auto; box-shadow:4px 0 20px rgba(0,0,0,0.3); }
                    ...
                    .profile-dropdown { position:fixed; top:50% !important; left:50% !important; transform:translate(-50%,-44%) !important; width:300px; background:#1C1C2E; border-radius:12px; padding:24px; z-index:100001; opacity:0; pointer-events:none; transition:opacity 0.22s ease,transform 0.22s ease; border:1px solid #2C2C44; box-shadow:0 24px 60px rgba(0,0,0,0.6); }
                ...
                <div id="ed-side-nav">
                    <div class="logo"><span style="color:white;font-weight:700;">EcoleDirecte</span></div>
                    <div class="tab-bar side-tab-bar">
                        ...
                        <button class="tab-btn" data-tab="viescolaire"><img src="${iconeVie}" width="16" height="16">Vie scolaire</button>
                        <button class="tab-btn" data-tab="messagerie"><img src="${iconeMessages}" width="16" height="16">Messages</button>
                        <button class="tab-btn" data-tab="apps"><img src="${iconeApps}" width="16" height="16">Apps</button>
                        <button class="tab-btn" data-tab="settings"><img src="${iconeParam}" width="16" height="16">Parametres</button>
                        <div class="profile-avatar side-profile-avatar">${pren.charAt(0)}${nom.charAt(0)}</div>
                    </div>
                </div>
                <div class="m-container">
                    <div id="top-tab-bar-container">${getTabBarHtml()}</div>
